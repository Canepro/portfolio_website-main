...
The flow: GitHub sends the webhook → the backend validates and persists the failed run → the agent pipeline runs → a PR or issue is created on the source repo. Five supported failure types at launch.
