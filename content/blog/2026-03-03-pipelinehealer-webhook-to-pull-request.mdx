...
During this run, the orchestrator polled for `ci-doctor` issues that correlated with the activity (`run_id`, `head_sha`, bounded time window) with an ~8 minute window and backoff.

**Update (v0.2.9):** External diagnostics now support `GH_AW_INGESTION_MODE=hybrid`, where passive `ci-doctor` findings and direct GitHub MCP context can coexist in one activity. The default wait budget is now 60 seconds with async backfill, and passive matching was hardened for label-drift cases.
