...
date: "2026-03-03"
description: "Real CI failure walkthrough: webhook gating, pattern diagnosis, fix PR, ci-doctor timing gap, and the backfill sweep."
...
For system context, see the [PipelineHealer architecture diagram](https://github.com/Canepro/pipelinehealer#architecture) and the [PipelineHealer repository](https://github.com/Canepro/pipelinehealer).
...
- **Pattern**: `dependency` (missing or unresolvable npm package)
- **Confidence**: high
- **Failure type**: `dependency`
- **Auto-fixable**: yes

No LLM call was needed. The model fallback covers ambiguous cases: unclear test assertions, unfamiliar build tools, odd config errors.

Pattern matching first. Model second. Every failure type that resolves with string matching should.
...
For this run configuration (`AUTO_CREATE_TRACKING_ISSUE_FOR_PRS=true`), it also filed [Issue #90](https://github.com/Canepro/pipelinehealer-demo/issues/90) as a tracking artifact with the full diagnosis context: failure type, confidence, evidence lines, and the remediation action taken.
...
During this run, the orchestrator polled for `ci-doctor` issues that correlated with the activity (`run_id`, `head_sha`, bounded time window) using the then-current ~8 minute window and backoff.

Current behavior supports `GH_AW_INGESTION_MODE=hybrid`, where passive `ci-doctor` findings and direct GitHub MCP context can coexist in one activity.

- Default wait budget is 60 seconds with async backfill.
- Passive matching is hardened for label-drift cases.
...
I built the backfill sweep after an E2E run showed `poll_window_exhausted` persisting even after `ci-doctor` finished. Component tests would not have caught the timing gap.
...
If I had only tested components in isolation, the shipped system would have:

- diagnosed and fixed failures correctly
- shown `poll_window exhausted` for external diagnostics permanently
- stored raw `ci-doctor` boilerplate
- rendered HTML comments in the UI

Each gap showed up on a deployed run, not in unit tests.
...
*Post 5 of 6 in the [AI Dev Days Hackathon](/blog) series.*
