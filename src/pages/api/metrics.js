...
let portfolioMetrics = {
  ...
  demo_clicks_chat: 0, // Clicks on chat/demo buttons (sandbox links may vary)
...
function toPrometheusFormat(metrics) {
  ...
  lines.push(
    `portfolio_engagement_metrics{type="contact_submissions"} ${metrics.contact_form_submissions}`
  );
  ...
  lines.push(
    `portfolio_performance_metrics{type="avg_page_load_time_ms"} ${metrics.avg_page_load_time}`
  );
