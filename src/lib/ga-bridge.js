...
class GoogleAnalyticsBridge {
  ...
  formatForPrometheus(gaMetrics) {
    ...
    lines.push(
      `ga4_performance_metrics{metric="page_load_time"} ${gaMetrics.performance.avg_page_load_time.toFixed(2)}`
    );
    lines.push(
      `ga4_performance_metrics{metric="first_contentful_paint"} ${gaMetrics.performance.first_contentful_paint.toFixed(2)}`
    );
    lines.push(
      `ga4_performance_metrics{metric="largest_contentful_paint"} ${gaMetrics.performance.largest_contentful_paint.toFixed(2)}`
    );
    lines.push(
      `ga4_performance_metrics{metric="cumulative_layout_shift"} ${gaMetrics.performance.cumulative_layout_shift.toFixed(3)}`
    );
    lines.push('');
    ...
    lines.push(
      `ga4_engagement_metrics{metric="contact_submissions"} ${gaMetrics.engagement.contact_form_submissions}`
    );
    lines.push(
      `ga4_engagement_metrics{metric="project_views"} ${gaMetrics.engagement.project_detail_views}`
    );
    lines.push(
      `ga4_engagement_metrics{metric="avg_time_on_page"} ${gaMetrics.engagement.avg_time_on_page}`
    );
    lines.push(
      `ga4_engagement_metrics{metric="return_visitors_pct"} ${(gaMetrics.engagement.return_visitors_percentage * 100).toFixed(2)}`
    );
    lines.push('');
